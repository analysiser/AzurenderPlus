...
#define MAX_THREADS 8
    ...
    bool Raytracer::mpiPathTrace(FrameBuffer &buffer)
    {
        ...
        if (scene->node_rank == 0)
            generateEyeRayList(eyeRays);
        
        azMPI mpiInstance(this->scene, buffer, this->width, this->height);
        mpiInstance.mpiPathTrace(eyeRays);
        return true;
    }
            ...
            if (isHit) {
                if (record.diffuse != Color3::Black() && record.refractive_index == 0)
                {
                    ...
                    for (size_t li = 0; li < scene->num_lights(); li++) {
                        ...
                        Vector3 samplePoint;
                        float tlight;
                        Color3 shadingColor = record.diffuse * aLight->SampleLight(record.position, record.normal, EPSILON, TMAX, &samplePoint, &tlight);
                        shadingColor *= INV_PI;
                        
                        // for each light sample
                        // we sample only one point per light for now
    ...
    void Raytracer::generateEyeRayList(vector<Ray> &eyerays)
    {
